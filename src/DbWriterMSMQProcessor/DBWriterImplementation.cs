...
{
    public class DotNetNancyAuditDBWriterImplementation
    {
        #region Properties
        ...
        static string _dbWriterQueueName;
        static int _messageThreshold;
        static ILog _log;
        #endregion

        #region Constructor
        static DotNetNancyAuditDBWriterImplementation()
        {
            try
            {
        ...
        public void Start()
        {
            try
            {
                ...
                string messageQueueSetting = ConfigurationManager.AppSettings["dbWriterQueueName"];
                _dbWriterQueueName = messageQueueSetting == null ? null : messageQueueSetting;

                if (_dbWriterQueueName == null)
                    throw new ApplicationException("No valid dbWriterQueueName setting in the Configuration File");

                // Create a transaction queue using System.Messaging API
                if (!MessageQueue.Exists(_dbWriterQueueName))
                    MessageQueue.Create(_dbWriterQueueName, true);

                //Connect to the queue
                MessageQueue Queue = new MessageQueue(_dbWriterQueueName);

                System.Messaging.MessageQueue.EnableConnectionCache = false;
                watchQueueDelegate dlgt = new watchQueueDelegate(watchQueue);
                IAsyncResult ar = dlgt.BeginInvoke(_dbWriterQueueName, new AsyncCallback(watchQueueCallback), dlgt);

                _log.Debug("DBWriterMSMQProcessingService is running");
        ...
        void _keepTabsOnQueueSizeCounter_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            MessageQueue q = null;
            string queuePath = _dbWriterQueueName;
            try
            {
